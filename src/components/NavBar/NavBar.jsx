...
const ThemeIcon = () => {
  ...
  return (
    <span onClick={handleMode}>
      {darkTheme ? (
        <img src={dayModeIcon} alt="Day Mode Theme" className="h-6 cursor-pointer transition-all dark:transition-all"/>
      ) : (
        <img src={nightModeIcon} alt="Night Mode Theme" className="h-6 cursor-pointer transition-all dark:transition-all"/>
      )}
    </span>
  );
};
...
function NavBar() {
  return (
  <nav className="fixed z-50 bg-cream w-full h-20 items-center flex drop-shadow-sm select-none transition-all dark:transition-all dark:bg-flint">
      
      <Link to="/" className='mr-auto'>
      <svg id="nameLogo" className="fill-current h-10 justify-start ml-6 cursor-pointer animate-spin-slow dark:text-cream" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250" >

...
function NavBar() {
  ...
  return (
    <nav>
      ...
      {/* Mobile Menu */}
      {mobileActive ? (null) : (
        <div className="h-screen w-screen bg-cream dark:bg-flint fixed justify-center items-center z-50">
          <div className="flex flex-col items-center">
          <svg onClick={toggleMobile} xmlns="http://www.w3.org/2000/svg" className="cursor-pointer absolute h-10 w-10 top-6 right-10 stroke-flint dark:stroke-cream" stroke="currentColor" viewBox="0 0 24 24" >
           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
            <Link to="/" onClick={toggleMobile} className="pt-20 text-6xl m-3 font-poppins font-bold cursor-pointer dark:text-cream hover:text-lavender dark:hover:text-lavender">
              <span>HOME</span>
            </Link>
            <Link to="/work" onClick={toggleMobile} className="text-6xl m-3 font-poppins font-bold cursor-pointer dark:text-cream hover:text-salmon dark:hover:text-salmon">
              <span >WORK</span>
            </Link>
            <Link to="/about" onClick={toggleMobile} className="text-6xl m-3 font-poppins font-bold cursor-pointer dark:text-cream hover:text-sunflower dark:hover:text-sunflower">
              <span>ABOUT</span>
            </Link>
            <Link to="/contact" onClick={toggleMobile} className="text-6xl m-3 font-poppins font-bold cursor-pointer dark:text-cream hover:text-mint dark:hover:text-mint">
              <span >CONTACT</span>
            </Link>
